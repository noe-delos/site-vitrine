...
const BentoCard = ({
  ...
  <div
    key={name}
    className={cn(
      'group relative flex flex-col justify-between overflow-hidden border-white rounded-xl cursor-pointer',
  ...
  >
    ...
    <div className="pointer-events-none z-10 border-white flex transform-gpu flex-col gap-1 p-4 sm:p-6 transition-all duration-300 group-hover:-translate-y-10">
      ...
      <h3
        className={cn(
          'text-lg sm:text-xl border-white font-semibold text-gray-900',
          textClassName,
        )}
      >
        {name}
      ...
      <p
        className={cn(
          'max-w-lg text-sm sm:text-base text-gray-500 border-white',
          textClassName,
        )}
      >
        {description}
